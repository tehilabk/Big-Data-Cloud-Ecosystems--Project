...
async function get_track_number() { // get a uniqe number for the package tracking number
  let rawdata = fs.readFileSync('../package_formation/track_num_count.json');
  let track_num = JSON.parse(rawdata);
  let data = JSON.stringify(track_num + 1);
  fs.writeFileSync('../package_formation/track_num_count.json', data);
  return track_num;
}
